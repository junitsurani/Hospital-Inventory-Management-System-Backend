...
async function deleteRow(req, res) {
    let dbResult = await DB.execute("DELETE FROM journal_voucher_master WHERE `JVM_ID`=?", [req.body.JVM_ID]);
    dbResult = dbResult[0];

    if (dbResult.affectedRows) {
        let journalVoucherMasterData = await DB.execute(`SELECT * FROM journal_voucher_master`);
        journalVoucherMasterData = journalVoucherMasterData[0];
